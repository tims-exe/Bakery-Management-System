...
import 'package:nissy_bakes_original/components/order_item_edit.dart';
import 'package:nissy_bakes_original/components/order_more_details.dart';
import 'package:nissy_bakes_original/components/search_item.dart';
import 'package:nissy_bakes_original/components/select_customer.dart';
...
class _OrderPageState extends State<OrderPage> {
  ...
  final TextEditingController finalPaymentFieldController =
      TextEditingController(text: '0');
  final TextEditingController discountFieldController = TextEditingController(
    text: '0',
  );
    ...
    for (Map<String, dynamic> items in details) {
      ...
      if (amount != 0) {
        if (formula == 'num_x_sellqnty') {
          ...
          invoiceItems =
              '$invoiceItems${(items['number_of_items'] * items['sell_quantity'])} ${_getItemName(items['item_id'])} @ Rs $amount/-\n';
        } else if (formula == 'num_x_sellqnty_unit') {
          //
          invoiceItems =
              '$invoiceItems${(items['number_of_items'] * items['sell_quantity'])} ${_getItemName(items['item_id'])} (${_getUnitId(items['sell_unit_id'])}) @ Rs $amount/-\n';
        } else {
          // num  name (sellqnty sellunit)
          invoiceItems =
              '$invoiceItems${items['number_of_items']} ${_getItemName(items['item_id'])} (${items['sell_quantity']} ${_getUnitId(items['sell_unit_id'])}) @ Rs $amount/-\n';
        }
      } else {
        if (formula == 'num_x_sellqnty') {
          // (number of items * sell qnty) name
          invoiceItems =
              '$invoiceItems${(items['number_of_items'] * items['sell_quantity'])} ${_getItemName(items['item_id'])}\n';
        } else if (formula == 'num_x_sellqnty_unit') {
          //
          invoiceItems =
              '$invoiceItems${(items['number_of_items'] * items['sell_quantity'])} ${_getItemName(items['item_id'])} (${_getUnitId(items['sell_unit_id'])})\n';
        } else {
          // num  name (sellqnty sellunit)
          invoiceItems =
              '$invoiceItems${items['number_of_items']} ${_getItemName(items['item_id'])} (${items['sell_quantity']} ${_getUnitId(items['sell_unit_id'])})\n';
        }
      }
    }
                      ...
                      TextButton(
                        onPressed: () {
                          if (getMobileNumber(currentBill['customer_id']) !=
                              '') {
                            mobileNumberFieldController.text = getMobileNumber(
                                  ...
                                  ElevatedButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      await _dbhelper.updateOrderHeader(
                                        'bill_sent',
                                        1,
                                        orderHeader['bill_number_type'],
                                        orderHeader['bill_number_financial_year'],
                                        orderHeader['bill_number'],
                                      );
                                      sendWhatsAppMessage(
